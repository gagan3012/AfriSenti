...
class ModelArguments:
    ...
    hyperparameter_search: bool = field(
        default=False,
        metadata={"help": "Will enable hyperparameter search"},
    )
...
def main():
    ...
    # training_args.output_dir = os.path.join(training_args.output_dir, model_args.model_name_or_path, model_args.data_subset)
    # training_args.logging_dir = os.path.join(training_args.logging_dir, model_args.model_name_or_path, model_args.data_subset)
    ...
    if training_args.do_train:
        ...
        df = pd.read_csv(model_args.data_dir +
                         'train/{}_train.tsv'.format(model_args.data_subset), sep='\t')
        df = df.dropna()
        print(df)
        dataset = Dataset.from_pandas(df)
        train_dataset = dataset #data_split(dataset=dataset)['train']
        label_list = df['label'].unique().tolist()
    ...
    if training_args.do_eval:
        ...
        df = pd.read_csv(model_args.data_dir +
                         'dev_gold/{}_dev_gold_label.tsv'.format(model_args.data_subset), sep='\t')
        df = df.dropna()
        dataset = Dataset.from_pandas(df)
        eval_dataset = dataset #data_split(dataset=dataset)['valid']   
        label_list = df['label'].unique().tolist()
    ...
    if training_args.do_predict:
        ...
        df = df.dropna()
        dataset = Dataset.from_pandas(df)
        predict_dataset = dataset #data_split(dataset=dataset)['test']
        label_list = df['label'].unique().tolist()
        # label_list = df['label'].unique().tolist()
    ...
    def remove_url_from_list(text_list):
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        #print
        texts =(remove_url_from_list(examples['tweet']),)
